...
这是小鱼个人网站的全站视觉与交互规范。工具只是网站中的一个内容分支；规范同时适用于 Astro 页面、原生工具、Electron 桌面外壳和私人控制中心。可执行真源位于 `packages/design-system/src`；业务页面只能消费语义 class 和 `--lm-*` token，不在页面内复制品牌值。
...
- 不使用复杂渐变、发光、实时图表或无目的的装饰性动画。首页可以使用一次性滚动揭示和轻量视差，但必须服务于内容理解。
...
- 动效只允许 opacity、transform 和颜色/边框过渡。控件反馈使用 120/180/240ms；首页叙事入场可延长到 900ms，并统一使用 `cubic-bezier(.22,1,.36,1)`。
- 滚动叙事保留浏览器原生滚动，只在 `requestAnimationFrame` 中更新可见元素；不接管滚轮，不添加全站惯性滚动。
